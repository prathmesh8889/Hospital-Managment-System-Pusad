    ...
    <>
      <Drawer open={open} onClose={onClose} width={430}>
        ...
        <div className="p-5 space-y-5">
          ...
          {role === "super" && (
            <section className="bg-pine-950 pine-tex text-white border border-pine-800 rounded-xl p-4 space-y-3">
              <div className="flex items-start gap-2.5">
                <span className="w-9 h-9 rounded-lg bg-white/10 text-brand-300 grid place-items-center shrink-0"><I name="shield" className="w-4 h-4" /></span>
                <div>
                  <h3 className="font-display font-bold text-[14px]">Confidential Ledger</h3>
                  <p className="text-[11px] text-pine-100/60 mt-0.5 leading-relaxed">Super Admin-only internal workspace. It is not shown in the normal sidebar and requires your ID and password again.</p>
                </div>
              </div>
              <div className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[10.5px] text-pine-100/60 leading-relaxed">
                For legitimate internal expenses, petty cash, reconciliation, write-offs, adjustments and sensitive notes. Official patient revenue remains in Billing.
              </div>
              <Btn variant="outline" icon="lock" className="w-full !border-white/20 !text-white hover:!bg-white/10" onClick={() => setLedgerOpen(true)}>Open confidential ledger</Btn>
            </section>
          )}
      ...
      <ConfidentialLedger open={ledgerOpen} onClose={() => setLedgerOpen(false)} />
    </>
  );
}
